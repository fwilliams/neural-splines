...
def cell_weights_trilinear(padded_cell_bbox, cell_bbox, voxel_size):
    ...
    padded_cell_vmin = np.round(padded_cell_bbox[0] / voxel_size).to(torch.int32)
    padded_cell_vmax = np.round((padded_cell_bbox[0] + padded_cell_bbox[1]) / voxel_size).to(torch.int32)
