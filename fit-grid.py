...
def reconstruct_on_grid(model, full_grid_size, full_bbox, cell_bbox, cell_bbox_normalized, scale, out,
    ...
    cell_vox_min = np.round(cell_bbmin_rel * full_grid_size).astype(np.int32) + 1
    cell_vox_max = np.round(cell_bbmax_rel * full_grid_size).astype(np.int32) + 1
...
def main():
    ...
    out_grid = np.zeros(full_grid_size, dtype=np.float32)
